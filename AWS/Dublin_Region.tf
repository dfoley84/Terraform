...
resource "aws_instance" "JenkinsMaster" {
    ...
    subnet_id = "${aws_subnet.Dublin_public_subnet.id}"
    security_groups = ["${var.JenkinMaster_SG_group}"]
    #<< ANSIBLE TOOL BUCKET>>
...
resource "aws_instance" "JenkinsSlaveNode1" {
     ami = "${var.Jenkin_AMI}"
     instance_type = "${var.instance_class}"
     key_name = "${var.key_name}"
     subnet_id = "${aws_subnet.Dublin_public_subnet.id}"
     security_groups = ["${var.JenkinSlave_SG_group}"]
      tags {
            name = "Jenkins Slave Node 1"
    }
}

resource "aws_instance" "JenkinsSlaveNode2" {
     ami = "${var.Jenkin_AMI}"
     instance_type = "${var.instance_class}"
     key_name = "${var.key_name}"
     subnet_id = "${aws_subnet.Dublin_public_subnet.id}"
     security_groups = ["${var.JenkinSlave_SG_group}"]
      tags {
            name = "Jenkins Slave Node 2"
    }
}

# TO DO 

# Create a Baston Host Move Jenkins to Private Subnet 
